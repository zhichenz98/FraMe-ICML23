...
import numpy as np
import ot
import frame.optim as optim
from frame.fgw_utils import dist,reshaper
from frame.bregman import sinkhorn_scaling
...
def fgw_barycenters(N,Ys,Cs,ps,lambdas,alpha,fixed_structure=False,fixed_features=False,p=None,loss_fun='square_loss', max_iter=100, tol=1e-9,verbose=False,log=True,init_C=None,init_X=None):
    ...
    for i in range(S):
        T[i] = T[i].T
        intra_cost =  0.5 * Cs[i]**2 @ ps[i].reshape((-1,1)) @ np.ones((1,N)) + 0.5 * np.ones((len(ps[i]),1)) @ p.reshape((1,-1)) @ (C**2).T - Cs[i] @ T[i] @ C.T
        fgw_cost = (1-alpha) * Ms[i].T + alpha * intra_cost
        FGW[i] = np.sum(fgw_cost * T[i])
    return X.T, C, T, FGW